...
if (q.includes(exam.name.toLowerCase()) || q.includes(exam.id.replace(/-/g," "))) {
      if (q.includes("cutoff")||q.includes("score")) { var c=exam.cutoff[0];
...
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent;}
  ...
  {0%{transform:scale(0);border-radius:50%}100%{transform:scale(50);border-radius:50%}}
...
return (
    <div style={{
      ...
      {/* Scanline sweep */}
      <div style={{position:"absolute",left:0,right:0,height:"1px",background:"linear-gradient(90deg,transparent 
0%,rgba(201,168,76,0.4) 40%,rgba(255,255,255,0.2) 50%,rgba(201,168,76,0.4) 60%,transparent 100%)",animation:"scanline 2.5s linear infinite",pointerEvents:"none",zIndex:5}}/>
...
export default function App() {
  var darkInit = false;
  try { darkInit = localStorage.getItem("examnest_dark")==="true";
} catch(e) {}
  ...
  <span style={{fontSize:16}}>{dark?"☀️":"🌙"}</span>
        ...
        <button onClick={function(){setNavTab("chat");}} style={{background:"linear-gradient(135deg,#6366f1,#ec4899)",border:"none",borderRadius:20,padding:"6px 12px",fontSize:11,color:"#fff",fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>🤖 AI</button>
              ...
              <div style={{position:"relative",marginBottom:12}}>
                ...
                <input value={search} onChange={function(e){setSearch(e.target.value);}} placeholder={"Search "+exams.length+"+ Indian exams..."} style={{width:"100%",padding:"11px 36px 11px 36px",background:"rgba(255,255,255,0.1)",border:"1px 
solid rgba(255,255,255,0.15)",borderRadius:12,color:"#fff",fontSize:14,fontFamily:"inherit"}}/>
...
style={{display:"flex",gap:5,flexWrap:"wrap"}}>
        ...
        <div style={{position:"fixed",bottom:0,left:0,right:0,background:T.navBg,borderTop:"1px solid "+T.border,display:"flex",zIndex:100}}>
          {[{id:"home",icon:"🏠",label:"Home"},{id:"planner",icon:"📅",label:"Planner"},{id:"compare",icon:"⚖️",label:"Compare"},{id:"chat",icon:"🤖",label:"AI Chat"},{id:"career",icon:"🎯",label:"Career"},{id:"about",icon:"ℹ️",label:"About"}].map(function(item){
            return 
(
  ...
  <div>
        ...
        {data.step===2&&planExam&&(
         ...
         <button onClick={function(){upd({step:1});}} style={{background:"none",border:"none",color:T.muted,cursor:"pointer",fontSize:13,fontFamily:"inherit",marginBottom:14,display:"flex",alignItems:"center",gap:5}}>← Change Exam</button>
            ...
            {data.examDate&&<div style={{background:dark?"#1e293b":"#f0fdf4",borderRadius:14,padding:"14px",marginBottom:14,border:"1px solid #22c55e40"}}><div style={{fontSize:13,color:"#22c55e",fontWeight:600,marginBottom:4}}>🎯 {daysLeft} days left!</div><div style={{fontSize:12,color:T.subtext}}>{daysLeft>=90?"Great!
Plenty of time 💪":daysLeft>=30?"Good time. Stay consistent! 📚":daysLeft>=7?"Time is short! Focus! ⚡":"Very few days! High-priority only!
🔥"}</div></div>}
...
var subDone=topics.filter(function(t){return data.progress[subject+"::"+t]===2;}).length;
            ...
            {pct===100&&<div style={{background:"linear-gradient(135deg,#14532d,#166534)",borderRadius:14,padding:"16px",textAlign:"center",marginTop:8}}><div style={{fontSize:36,marginBottom:8}}>🎉</div><div style={{fontFamily:"'Playfair Display',serif",fontSize:18,fontWeight:700,color:"#fff",marginBottom:4}}>All Topics Completed!</div><div style={{fontSize:13,color:"rgba(255,255,255,0.8)"}}>Now focus on revision and mock tests!
💪</div></div>}
    ...
    <div style={{minHeight:"100vh",background:T.bg,paddingBottom:30,color:T.text}}>
      <div style={{background:exam.color,padding:"14px 16px 0",position:"sticky",top:0,zIndex:50}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:13,marginBottom:14}}>
          ...
          <div>
            <div 
style={{fontSize:10,color:"rgba(255,255,255,0.7)",textTransform:"uppercase",marginBottom:2}}>{exam.category}</div>
...
function AboutPage(props) {
  ...
  return(
    <div style={{minHeight:"100vh",background:T.bg,paddingBottom:80,color:T.text}}>
      ...
      <div style={{padding:"16px"}}>
        {[
          {icon:"📚",title:"Who We Are",text:"ExamNest is India's most complete free exam platform — covering "+count+"+ exams across Engineering, Medical, UPSC, SSC, Banking, Railway, Defence, Management, Law, 
Commerce & more!",color:"#f97316"},
          ...
          {icon:"🤖",title:"AI Chatbot",text:"ExamBot knows all Indian exams — ask about syllabus, cutoffs, books, tips, eligibility instantly.
Works offline!",color:"#6366f1"},
          ...
          {icon:"📅",title:"Study Planner",text:"Personalised weekly plan + topic tracker.
Progress saved permanently — never resets even if you close the app!
🔒",color:"#16a34a"},
          {icon:"🌙",title:"Dark Mode",text:"Easy on eyes for night studying!
Tap the moon icon in the top bar to switch between light and dark mode.",color:"#7c3aed"},
        ...
        <div style={{textAlign:"center",marginTop:16,fontSize:12,color:T.muted}}>© 2026 ExamNest · {count}+ 
Exams · Made with ❤️ for Indian Students</div>
      </div>
    </div>
  );
}
